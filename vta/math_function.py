...
def trigonometry(text):
	...
	if 'sin' in text:
		...
		return round(1/round(math.sin(rad),2))
	elif 'sec' in text:
		return round(1/round(math.cos(rad),2))
	elif 'cot' in text:
		return round(1/round(math.tan(rad),2))
...
def perform(text):
	...
	elif isHaving(text, ['sin','cos','tan','cosec','sec','cot']): return str(trigonometry(text))
